...
/* return max size of Free Data blocks of an Empty VM page */
...
mm_union_free_blocks (block_meta_data_t *first, block_meta_data_t *second) {
    /* check first and second merged blocks */
    assert(first->is_free == MM_TRUE && second->is_free == MM_TRUE);

    /* Update fist block's size */
    first->block_size += sizeof(block_meta_data_t) + second->block_size;

    /* update first block's pointer to next block */
    first->next_block = second->next_block;

    /* update the next block's pointer to prev block */
...
vm_page_t *
mm_allocate_vm_page(vm_page_family_t *vm_page_family) {

    vm_page_t *vm_page = mm_get_new_vm_page_from_kernel(1);

    /* initialise the first meta block in this vm page */
    MARK_VM_PAGE_EMPTY(vm_page);
    /* initialise the size of this meta block */
    vm_page->block_meta_data.block_size = mm_max_page_allocatable_memory(1);
    /* initialise the offset of this meta block */
    vm_page->block_meta_data.offset = offset_of(vm_page_t, block_meta_data);
    /* update to free_blocks_list */
    init_glthread(&vm_page->block_meta_data.priority_thread_glue);
    vm_page->next = NULL;
    vm_page->prev = NULL;

    /* Set the pointer to this vm page's page family */
    vm_page->pg_family = vm_page_family;

    /* If this vm page is NOT the first vm page for a given vm_page_family */
    if(vm_page_family->first_page) {

        /* Insert this new vm page to the head of the linked list */
...
mm_vm_page_delete_and_free(
    ...
    /* If this deleted vm page is the head of thee linked list */
    ...
    /* If this deleted vm page is the middle or end of linked list */
    if(vm_page->next)
...
mm_instantiate_new_page_family(
    ...
    /* If the current vm page for families is full 
        => create a new vm page for coming family*/
...
mm_allocate_free_data_block(
    ...
    /* look for the bigest free data block : Worst Fit allocations */
...
xcalloc(char *struct_name, int units){
    ...
    /* look for struct_name in vm page for families */ 
    ...
    /* check the requested size exceeds the MAX allocated size of vm page */
    ...
    if(free_block_meta_data){
        ...
        /* return the address of the starting of  data block for use */
